...
async function init() {
  ...
  try {
    ...
    console.log('[APP] Initializing timers...');
    initTimers();
...
async function selectTeamForSlot(teamName) {
  ...
  if (isTimerExpired(stageId)) {
    alert('⚠️ Время приёма ставок истекло!');
    closeModal();
    return;
  }
...
function setupSlotClicks() {
  ...
  stageContent.addEventListener('click', function(e) {
    ...
    if (isTimerExpired(stageId)) {
      console.log('[APP] Timer expired for stage:', stageId);
      alert('⚠️ Время приёма ставок истекло!');
      return;
    }
...
function updateSaveButton(stageId) {
  ...
  const isClosed = !isStageOpen(stageId);
  btn.disabled = !hasPicks || isTimerExpired(stageId) || isClosed;
}
